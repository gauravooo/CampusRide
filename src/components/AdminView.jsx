...
import { Shield, Map, BrainCircuit, Award, Layers, RefreshCw, Zap, CheckCircle2, AlertTriangle } from 'lucide-react';
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/10 pb-4">
        <div>
          <h2 className="text-xl font-black text-white flex items-center gap-2">
            ...
            <span>Campus Fleet Command Dashboard</span>
          </h2>
          <p className="text-xs text-slate-400">Real-time IoT Telemetry & AI Fleet Operations for IIM Bodh Gaya</p>
        </div>

        <button
          onClick={handleTriggerRebalance}
          disabled={rebalanceStatus === 'optimizing'}
          className="px-4 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-2 shadow-lg shadow-purple-600/30 transition"
        >
          <BrainCircuit className="w-4 h-4 text-purple-300" />
          <span>{rebalanceStatus === 'optimizing' ? 'Running AI Optimization...' : 'Run AI Fleet Rebalancing'}</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <div className="glass-card p-4 rounded-2xl border-white/10">
          ...
          <p className="text-[10px] text-blue-400 mt-1 font-mono">24.6961° N Geofenced</p>
        </div>
        <div className="glass-card p-4 rounded-2xl border-white/10">
          ...
          <p className="text-[10px] text-emerald-400/80 mt-1">Real-time Telemetry</p>
        </div>
        <div className="glass-card p-4 rounded-2xl border-white/10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-card p-4 rounded-3xl space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400">10 Station Hubs • 200 Cycles</span>
          </div>
        ...
        <div className="glass-card p-4 rounded-3xl flex flex-col space-y-3">
          ...
          <p className="text-xs text-slate-400">Predictive peak hub deficits based on hourly class schedules & hostel demand.</p>
      ...
      {/* Student Leaderboard & Hub Inventory Tables */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Student Trust Leaderboard */}
        <div className="glass-card p-4 rounded-3xl space-y-3">
          <div className="flex items-center justify-between border-b border-white/10 pb-2">
            <h2 className="text-sm font-extrabold text-white flex items-center gap-2">
              <Award className="w-5 h-5 text-amber-400" />
              <span>Student Trust Score Leaderboard</span>
            </h2>
            <span className="text-xs text-slate-400">@iimbg.ac.in Domain</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-900 text-slate-400 uppercase text-[10px] font-bold">
                <tr>
                  <th className="p-2.5 rounded-l-xl">Student</th>
                  <th className="p-2.5">Email</th>
                  <th className="p-2.5">Trust Rating</th>
                  <th className="p-2.5 rounded-r-xl">Role</th>
                </tr>
              </thead>
        ...
        <div className="glass-card p-4 rounded-3xl space-y-3">
          <div className="flex items-center justify-between border-b border-white/10 pb-2">
            <h2 className="text-sm font-extrabold text-white flex items-center gap-2">
              ...
              <span>Station Capacity & Inventory</span>
            </h2>
          ...
          <div className="overflow-x-auto max-h-72">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-900 text-slate-400 uppercase text-[10px] font-bold">
                <tr>
                  <th className="p-2.5 rounded-l-xl">Station Hub</th>
                  <th className="p-2.5">Code</th>
                  <th className="p-2.5">Available</th>
                  <th className="p-2.5">Capacity</th>
                  <th className="p-2.5 rounded-r-xl">Fill %</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800">
                {hubs.map((h) => {
                  ...
                  return (
                    <tr key={h.id} className="hover:bg-slate-800/40 transition">
                      <td className="p-2.5 font-bold text-white">{h.name}</td>
                      <td className="p-2.5 font-mono text-blue-400">{h.code}</td>
                      <td className="p-2.5 font-extrabold text-emerald-400">{avail}</td>
